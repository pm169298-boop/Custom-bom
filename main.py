...
BOOT_BRAND = "🥷 ◤" + sc("droid") + "◢"
BOOT_TYPE_TEXT = sc("system starting")  # typing effect mein char-by-char dikhata hai
START_LOADING_TEXT = f"{BOOT_BRAND}\n\n⏳"
...
async def _boot_animation(boot_msg, done_flag: list, edit_lock, ready_event: asyncio.Event):
    """TYPEWRITER effect: brand turant, phir 'system starting' letter-by-letter type hota hai,
    end pe ⚙️. Final panel aate hi band (done_flag). Edit lock se race nahi."""
    if boot_msg is None:
        ready_event.set()
        return
    base = f"{BOOT_BRAND}\n\n"
    full = base + BOOT_TYPE_TEXT
    for n in range(1, len(BOOT_TYPE_TEXT) + 1):
        await asyncio.sleep(0.055)  # ~1 char/55ms — typing ki feel
        async with edit_lock:
            if done_flag[0]:
                ready_event.set()
                return
            try:
                await boot_msg.edit_text(base + BOOT_TYPE_TEXT[:n])
            except Exception:
                ready_event.set()
                return
    ...
    async with edit_lock:
        if done_flag[0]:
            ...
            return
        try:
            await boot_msg.edit_text(full + " ⚙️")
        except Exception:
            pass
    ready_event.set()
...
async def cmd_start_deep(msg: Message, state: FSMContext):
    ...
    # INSTANT boot message — DB work se pehle bhejo (bot turant zinda lage; final panel isi mein edit hoga)
    try:
        boot_msg = await msg.answer(START_LOADING_TEXT)
    except Exception:
        boot_msg = None
    # typewriter animation: 'system starting' letter-by-letter type hota hai
    boot_done = [False]
    ...
    if is_owner(uid, d):
        await _boot_edit_or_send(msg, boot_msg, boot_done, boot_lock, boot_ready, owner_panel_text(d), owner_kb(d))
        return
    if is_admin(uid, d):
        await _boot_edit_or_send(msg, boot_msg, boot_done, boot_lock, boot_ready, admin_panel_text(d), admin_kb(d))
        return
    if is_banned(uid, d):
    ...
    await _boot_edit_or_send(msg, boot_msg, boot_done, boot_lock, boot_ready, user_home_text(uid, d), user_kb())

...
async def _welcome(msg: Message, state: FSMContext, ref_code: str = ""):
    ...
    async def _media_then_panel(text: str, panel_kb=None):
        """Sequence: boot animation khatam → video → panel NAYA message (video ke NEECHE = video upar)
        → panel animated reveal → boot msg DELETE (chat clean: video + panel)."""
        try:
        ...
        # Panel NAYA message — pehle 3 lines, phir animated reveal
        first_chunk = "\n".join(text.split("\n")[:3])
        try:
            ...
            panel_msg = None
        if panel_msg is None:
            # fallback: boot msg hi panel ban jaye
            await _boot_edit_or_send(msg, boot_msg, boot_done, boot_lock, boot_ready, text, panel_kb)
            return
        ...
        if not ok:
            try:
                ...
                pass
        # Boot msg delete — chat mein sirf video + panel (video panel ke UPAR)
        if boot_msg is not None:
            try:

...
START_LOADING_TEXT = "⏳ loading."  # instant pehla message (phir animation: loading.. → ... → words)
...
async def send_butterfly_effect_private(bot: Bot, chat_id: int):
    """Welcome effect: ek butterfly emoji bhejo, 2s ruko, phir delete karo
    (aag wale effect ka same pattern, sirf emoji butterfly hai)."""
    try:
...
async def _boot_animation(boot_msg, done_flag: list, edit_lock, ready_event: asyncio.Event):
    """WORD-BY-WORD animation: pehle 'loading...' dots, phir brand words ek-ek karke, end pe ⚙️.
    Final panel aate hi band (done_flag). Edit lock se race nahi."""
    if boot_msg is None:
        ready_event.set()
        return
    words = " ".join(BOOT_WORDS)
    # WORD SWAP: loading ek baar → droid ek baar → system ek baar (naya word aate hi purana chala jaata hai)
    stages = [
        "⏳ loading..",
        "⏳ loading...",
        "⏳ " + BOOT_WORDS[0],
...
async def _animate_panel(boot_msg, done_flag: list, edit_lock, text: str, reply_markup=None) -> bool:
    """PANEL ANIMATED open: pehle text line-by-line reveal, phir buttons row-by-row (premium feel).
    boot_msg nahi hai / edit fail → False (caller simple edit se fallback karega)."""
    if boot_msg is None:
        return False
    lines = text.split("\n")
    rows = list(reply_markup.inline_keyboard) if reply_markup is not None else []
    # Phase 1: text reveal (2 line/edit — snappy + kam edits)
    i = 0
    while i < len(lines):
        i = min(i + 2, len(lines))
        ...
        async with edit_lock:
            if done_flag[0]:
                return True
            try:
                await boot_msg.edit_text("\n".join(lines[:i]), parse_mode="HTML", disable_web_page_preview=True)
            except Exception:
                return False
    # Phase 2: buttons row-by-row
    for i in range(1, len(rows) + 1):
        ...
        async with edit_lock:
            if done_flag[0]:
                return True
            try:
                await boot_msg.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows[:i]), parse_mode="HTML", disable_web_page_preview=True)
            except Exception:
                return False
    return True
...
async def _welcome(msg: Message, state: FSMContext, ref_code: str = ""):
    ...
    async def _media_then_panel(text: str, panel_kb=None):
        """Sequence guarantee: animation khatam → video → final panel ANIMATED reveal (isi boot msg mein)."""
        try:
        ...
        ok = await _animate_panel(boot_msg, boot_done, boot_lock, text, panel_kb)
        if not ok:

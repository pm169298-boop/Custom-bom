...
def has_unlimited(uid: int, d: dict) -> bool:
    """Active duration-plan access (e.g. Monthly Unlimited) — unlimited_until tak unlimited use."""
    try:
...
def user_home_text(uid: int, d: dict) -> str:
    ...
    scan_info = get_scan_status()
    unlimited_row = f"🔓  {sc('unlimited')} : <b>{unlimited_days_left(uid, d)} {sc('days left')}</b>\n" if has_unlimited(uid, d) else ""
    return (
...
async def user_speed_selected(cq: CallbackQuery, state: FSMContext):
    ...
    if not is_admin(uid, d) and not is_owner(uid, d):
        if has_unlimited(uid, d):
            credit_info = f"\n{em(EMOJI_CHECK, '🔓')} <b>Unlimited access</b> — {unlimited_days_left(uid, d)} {sc('days left')} (koi credit nahi lagega)\n"
        else:
...
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    ...
    if is_regular_user:
        if unlimited_user:
            credit_text = f"\n{em(EMOJI_CHECK, '🔓')} Unlimited Access — 0 {sc('credits used')} ({unlimited_days_left(uid, load())} {sc('days left')})"
        else:
...
async def user_credits(cq: CallbackQuery, state: FSMContext):
    ...
    unlimited_row = f"🔓  <b>{sc('unlimited active')}</b> — {unlimited_days_left(uid, d)} {sc('days left')}\n" if has_unlimited(uid, d) else ""
    await cq.message.edit_text(
        f"💳 ◤{sc('credits')}◢\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
